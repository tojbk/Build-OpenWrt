...
# 后台IP设置
export Ipv4_ipaddr="192.168.123.254"        # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="0"                          # 修改主机名称为OpenWrt-123(填0为不作修改)

# 默认主题设置
export Mandatory_theme="atmaterial_new"     # 将bootstrap替换您需要的主题为必选主题(可自行更改您要的,源码要带此主题就行,填写名称也要写对) (填写主题名称,填0为不作修改)
export Default_theme="atmaterial_new"       # 多主题时,选择某主题为默认第一主题 (填写主题名称,填0为不作修改)
...
export Customized_Information="gd772 $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
# 修改插件名字
sed -i 's/"WireGuard 状态"/"WiGd状态"/g' `egrep "WireGuard 状态" -rl ./`
sed -i 's/"Web 管理"/"Web管理"/g' `egrep "Web 管理" -rl ./`
sed -i 's/"管理权"/"改密码"/g' `egrep "管理权" -rl ./`
sed -i 's/"TTYD 终端"/"命令行"/g' `egrep "TTYD 终端" -rl ./`
sed -i 's/"KMS 服务器"/"KMS激活"/g' `egrep "KMS 服务器" -rl ./`
sed -i 's/"aMule设置"/"电驴下载"/g' `egrep "aMule设置" -rl ./`
sed -i 's/"广告屏蔽大师 Plus+"/"广告屏蔽"/g' `egrep "广告屏蔽大师 Plus+" -rl ./`
sed -i 's/"USB 打印服务器"/"打印服务"/g' `egrep "USB 打印服务器" -rl ./`
sed -i 's/"Frp 内网穿透"/"Frp 客户端"/g' `egrep "Frp 内网穿透" -rl ./`
sed -i 's/"网络存储"/"NAS"/g' `egrep "网络存储" -rl ./`
sed -i 's/"阿里云盘 WebDAV"/"阿里云盘"/g' `egrep "阿里云盘 WebDAV" -rl ./`
sed -i 's/"挂载 SMB 网络共享"/"挂载共享"/g' `egrep "挂载 SMB 网络共享" -rl ./`
sed -i 's/"IPSec VPN 服务器"/"IPSec 服务"/g' `egrep "IPSec VPN 服务器" -rl ./`
sed -i 's/"PPTP VPN 服务器"/"PPTP 服务"/g' `egrep "PPTP VPN 服务器" -rl ./`
